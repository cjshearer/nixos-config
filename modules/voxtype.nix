      ...
      home-manager.sharedModules = [
        (
          ...
          let
            ...
            # which session to monitor for hotkeys, and where emulated input events should be sent.
            voxtypeWrapper = pkgs.writeShellScript "voxtype-daemon" ''
              echo $PATH
              echo $WAYLAND_DISPLAY
              echo $DISPLAY
              exec ${lib.getExe pkgs.voxtype-vulkan} daemon
            '';
          in
          lib.mkIf config.services.voxtype.enable (
            lib.mkMerge [
              {
                systemd.user.services.voxtype.Service.ExecStart = lib.mkForce "${voxtypeWrapper}";
              ...
              (lib.mkIf osConfig.wsl.enable {
                ...
                # By using both voxtype's clipboard mode and a systemd user service that monitors for
                # finished transcriptions, we can trigger a paste in Windows, simulating the behavior
                # of voxtype's paste mode without going to the trouble of finding some way to make
                # wtype work in WSLg.
